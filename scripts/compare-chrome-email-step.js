...
const PROFILE_BASE_DIR = process.argv[3]
  ...
  : path.resolve("artifacts/chrome-email-profiles");
const RUN_DIR = path.resolve(
  "artifacts",
  `chrome-email-step-${new Date().toISOString().replace(/[:.]/g, "-")}`
);

const CASES = [
  { id: "chrome-no-stealth", stealth: false, profileDir: path.join(PROFILE_BASE_DIR, "chrome-no-stealth") },
  { id: "chrome-stealth", stealth: true, profileDir: path.join(PROFILE_BASE_DIR, "chrome-stealth") }
];
