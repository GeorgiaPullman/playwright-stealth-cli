...
function bundledPlaywrightRoot() {
  return path.dirname(require.resolve("playwright/package.json", { paths: [cliPackageRoot()] }));
}

function bundledPlaywrightCliPath() {
  return path.join(bundledPlaywrightRoot(), "cli.js");
}
...
function bundledCoreRequire() {
  const playwrightCoreRoot = path.dirname(require.resolve("playwright-core/package.json", { paths: [cliPackageRoot()] }));
  return require(playwrightCoreRoot);
}
...
async function runBundledPlaywrightCli(argv) {
  const cliPath = bundledPlaywrightCliPath();
  const child = spawn(process.execPath, [cliPath, ...argv.slice(2)], {
    cwd: process.cwd(),
    stdio: "inherit"
  });
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("close", (code, signal) => {
      if (signal) {
        reject(new Error(`Bundled Playwright CLI exited with signal ${signal}`));
        return;
      }
      if (code && code !== 0) {
        reject(new Error(`Bundled Playwright CLI exited with code ${code}`));
        return;
      }
      resolve();
    });
...
async function run(argv = process.argv) {
  ...
  if (commandName === "install") {
    await runBundledPlaywrightCli(argv);
    return;
  }
