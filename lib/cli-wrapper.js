...
function printProfileStatus(profileDir, options = {}) {
  const resolvedProfileDir = path.resolve(profileDir);
  const holders = profileUsage(resolvedProfileDir);
  if (options.json) {
    console.log(JSON.stringify({
      profile: resolvedProfileDir,
...
function addProfileCommands(program) {
  ...
  program.command("profile-status <directory>").description("check whether a profile directory is currently in use").option("--json", "output machine-readable JSON").action((directory, options) => {
    printProfileStatus(directory, options);
  });
}
